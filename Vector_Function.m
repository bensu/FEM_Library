classdef Vector_Function < hgsetget
    properties
        component_function
        dofs_per_component
        n_components
    ...
    methods
        function fun_out = Vector_Function(filler, ... 
                        n_components_in, dofs_per_component_in)
            % Always stored in component_function and element function
            component_list_in = repmat(filler,n_components_in, ...
                            dofs_per_component_in);
            fun_out.set('dofs_per_component',dofs_per_component_in)
            fun_out.set('component_function',component_list_in)
            fun_out.set('n_components',n_components_in)
            fun_out.set('zero_val',filler);
        end
        
        %% I/O
        function dof_list_in(fun,component_list)
            fun_in = reshape(component_list,fun.get('dofs_per_component'), ...
                        fun.get('n_components'))';
        ...
        function edit_component_by_id(vector,component_id,vector_in)
            ...
            require(max(component_id)<=vector.n_components, ...
                                        'Component too big')
        ...
        function add_function(vector,function_in)
            vector.set('component_function',vector.get('component_function')+function_in);
        end
