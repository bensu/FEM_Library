classdef Mesh < hgsetget
    ...
    methods (Static)
        ...
        function obj = meshgen(A,M,E,nu,rho)
            ...
            obj = Mesh(coordinates,connections,mat1);
        end
    end
    
    methods

        function obj = Mesh(coordinates,connections,material)
        ...
        function outfaces = outface(mesh)
            connections = mesh.get('connections');
            nodelist = [];
            par_plot = [];
            for nodenum = 1:mesh.nnodes()
                elelist = mesh.elementsofnode(nodenum);
                element = mesh.elementcreate(elelist(1));
                if length(elelist) < 8
...
%             big_elelist = unique(big_elelist);
        ...
        function element = elementcreate(obj,ele)
            ...
            element = Mech_H8(nodos,obj.get('material'));   % HARDCODED
        end
